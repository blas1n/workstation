...
## F. 설치 헬퍼 — 형님이 돌리는 유일한 수동 단계

`scripts/setup-e2e-credential.sh` (가드: `tests/test_e2e_credential_setup_guard.sh`, 12 단언)

- [x] 비밀이 **argv 로 안 간다** (`security -w` 를 인자 없이 → security 가 직접 두 번 묻는다)
- [x] 비밀이 셸 변수·출력에 안 들어간다
- [x] 넣은 뒤 **데몬이 하는 그 호출로 읽어서 확인**한다 (값은 버리고 rc 만)
- [x] rc 해석을 **러너와 공유**한다 (두 벌이 되면 덜 쓰이는 쪽이 썩는다)
- [x] `launchctl managername` 으로 **세션을 먼저 확인**한다
- [x] 전선 절단 5회 — 각각 자기 것만 빨강, 실행 수 12 고정
- [x] **잘못된 세션(Background)에서 실제로 돌려봤다**: 경고 → 잠김 감지 → 2단계에서
      중단(exit 1) → **keychain 은 그대로**(rc=44)

⚠️⭐ **이 가드의 첫 판본은 네 절단 중 셋을 놓쳤다.** 전부 같은 이유 — 메커니즘이 아니라
**텍스트**를 물었다: `-w` 가 `\` 뒤 다음 줄에 있었고, `lib/keychain_credential.sh` 가
`# shellcheck` **주석**에도 있었고, `launchctl managername` 이 **`echo` 문자열**에도
있었다. 지금은 스크립트를 **코드 뷰**(전체 줄 주석 제거 + 줄 이음)로 정규화하고
호출은 **호출 형태**(`$(launchctl ...`, `^\s*\.\s+.*lib`)로 못박는다.
